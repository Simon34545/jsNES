class nes2C02 {
	...
	GetColorFromPaletteRam(palette, pixel) {
		return this.palScreen[this.ppuRead(0x3F00 + ((palette << 2) & 0xFF) + pixel)];
	}
	...
	clock() {
		...
		this.sprScreen.SetPixel(this.cycle - 1, this.scanline, this.GetColorFromPaletteRam(bg_palette, bg_pixel));//this.palScreen[(Math.floor(Math.random() * 32768) % 2) ? 0x3F : 0x30]);
	...
	cpuRead(addr, rdonly = false) {
		...
		case 0x0007: // PPU Data
			...
			if (this.ppu_address > 0x3f00) data = this.ppu_data_buffer;
			this.ppu_address++; */
			break;
		}
	...
	cpuWrite(addr, data) {
		...
		case 0x0006: // PPU Address
			if (this.address_latch == 0) {
				...
				//this.ppu_address = (this.ppu_address & 0x00FF) | (data << 8);
				this.address_latch = 1;
			} else {
		...
		case 0x0007: // PPU Data
			...
			//this.ppu_address += (this.control.increment_mode ? 32 : 1);
			break;
		}
	}
