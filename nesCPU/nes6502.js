...
class nes6502 {
	...
	clock() {
		if (this.cycles.v == 0) {
			...
			if (additionalCycle2 == undefined) {
				alert("Instruction " + this.lookup[this.opcode.v].name + " did not return a value!!!"); 
			}
	...
	ZPX() {
		this.addr_abs.v = (this.read(this.pc.v) + this.pc.x.v);
		this.pc.v++;
		this.addr_abs.v &= 0x00FF;
		return 0;
	}
	...
	ZPY() {
		this.addr_abs.v = (this.read(this.pc.v) + this.pc.y.v);
		this.pc.v++;
		this.addr_abs.v &= 0x00FF;
		return 0;
	}
	...
	PLP() {
		this.stkp.v++;
		this.status.v = this.read(0x0100 + stkp);
		this.SetFlag(this.FLAGS6502.U, 1);
		return 0
	}
	...
	disassemble(start, stop) {
		...
		while (addr <= stop) {
			...
			let opcode = this.bus.read(addr, true); addr++;
